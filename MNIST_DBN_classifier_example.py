...
if __name__ == '__main__':
	...
	torch.save(completed_model, 'mnist_trained_dbn_classifier_epoch50.pt')
	...
	progress.to_csv('DBN_without_pretraining_classifier_epoch50.csv', index=False)

	print("With Pre-Training")
	model, progress = train(completed_model, train_x, train_y, train_x, train_y, test_x, test_y)
	progress = pd.DataFrame(np.array(progress))
	progress.columns = ['epochs', 'test loss', 'train loss', 'test acc', 'train acc']
	progress.to_csv('DBN_with_pretraining_classifier_epoch50.csv', index=False)